(* https://github.com/paurkedal/caqti-study/blob/a2e07123e2fff689ff22de60ba33a6875e65f50b/study/bikereg-eio/bikereg.ml *)
 ...
 * code-generators. *)
 ...
 let report_stolen (module Db : Caqti_eio.CONNECTION) frameno =
   Db.exec Q.report_stolen ("2024-05-01T09:05:00", frameno)
 ...
 let test stdout db =
   ...
   iter_s_stolen db
     (fun bike ->
       let stolen =
         match bike.Bike.stolen with Some x -> x | None -> assert false
       in
       Printf.printf "\t%s %s %s\n" bike.Bike.frameno (Ptime.to_rfc3339 stolen) bike.Bike.owner;
       Ok ()
     )

  let () = Eio_main.run @@ fun env ->
    let url = Uri.of_string (Sys.getenv "DB_URL") in
